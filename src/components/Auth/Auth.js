...
const AuthFC = () => {
  ...
  const { authLoading, authFailedMsg } = useSelector(state => state)

  const [state, setState] = useState({
    mode: 'Sign Up'
  })

  const handlerSwitchMod = () => {
    setState({
      mode: state.mode === "Sign Up" ? "Login" : "Sign Up"
    })
  }
  ...
  if (authLoading) {
    ...
    form = (
      <Formik initialValues={{
        ...
        onSubmit={({ email, password }) =>
          dispatch(auth(email, password, state.mode))
        }
        validate={(values) => {
          const error = {}
          if (!values.email) {
        ...
        {({ values, handleChange, handleSubmit, errors }) =>
          ...
          >
            <button className='btn mb-3'
              ...
              onClick={handlerSwitchMod}
            >
              Switch to {
                state.mode === 'Sign Up' ? 'Login' : 'Sign Up'
              }
            </button>
            <form
              onSubmit={handleSubmit}
            ...
            >
              ...
              <button
                ...
                className='btn btn-success mt-3'
              >
                {state.mode === "Sign Up" ? "Sign Up" : "Login"}
              </button>
            </form>
          </div>}
      </Formik>
